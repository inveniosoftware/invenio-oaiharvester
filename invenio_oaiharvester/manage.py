...
"""
The main invenio-oaiharvester function, which can be used to harvest records from an OAI repository,
...
@manager.option('-d', '--output_dir', dest='output_dir', default=None)
@manager.option('-w', '--workflow', dest='workflow', default=None)
def get(metadata_prefix, name, setSpec, identifiers, from_date, until_date, url, output_dir, workflow):
    """
    ...
    begin_harvesting_action(metadata_prefix, name, setSpec, identifiers, from_date,
                            until_date, url, output_dir, workflow, is_queue=False)
...
@manager.option('-d', '--output_dir', dest='output_dir', default=None)
@manager.option('-w', '--workflow', dest='workflow', default=None)
def queue(metadata_prefix, name, setSpec, identifiers, from_date, until_date, url, output_dir, workflow):
    """
    ...
    """
    begin_harvesting_action(metadata_prefix, name, setSpec, identifiers, from_date,
                            until_date, url, output_dir, workflow, is_queue=True)
...
def begin_harvesting_action(metadata_prefix, name, setSpec, identifiers, from_date,
    ...
    if identifiers is None:
        ...
        callback(metadata_prefix, from_date, until_date, url, name, setSpec, output_dir, workflow)
    else:
        ...
        callback(identifiers, metadata_prefix, url, name, output_dir, workflow)
